...
class RectSparsePLKConv2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # No reparametrization since this is for a ablative study
        if self.training:
            ...
            x[:, :self.idx] = self.mn_conv(x[:, :self.idx]) + self.nm_conv(x[:, :self.idx]) + self.nn_conv(
                x[:, :self.idx])
            return x
...
class SparsePLKConv2d(nn.Module):
    ...
    def __init__(self, dim, max_kernel_size, sub_kernel_sizes, dilations, use_max_kernel, with_idt):
        ...
        convs = [
            nn.Conv2d(dim, dim, sub_kernel_size, 1, (sub_kernel_size // 2) * d, dilation=d) for sub_kernel_size, d in
            zip(sub_kernel_sizes, dilations)
        ]
        if use_max_kernel:
            convs.append(
...
class PLKBlock(nn.Module):
    def __init__(
            ...
            # Sparse Rep options
            use_max_kernel: bool = False, sparse_kernels: Sequence[int] = [5, 5, 5],
...
class PLKSR(nn.Module):
    def __init__(
        ...
        self.feats = nn.Sequential(*[
                                        ...
                                        PLKBlock(
                                            dim, ccm_type, kernel_size, split_ratio, lk_type, use_max_kernel,
                                            sparse_kernels, sparse_dilations, with_idt, use_ea
